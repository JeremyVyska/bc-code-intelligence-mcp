...
describe('WorkflowSessionManagerV2', () => {
  ...
  describe('Workflow Types', () => {
    ...
    it.each(workflowTypes)('should support workflow type: %s', async (workflowType) => {
      const session: WorkflowSession = {
  ...
  describe('Next Action Generation', () => {
    ...
    it('should return complete_workflow when all files processed', async () => {
      const session: WorkflowSession = {
        ...
        file_inventory: [
          {
            ...
            status: 'completed',
            checklist: [
              { id: 'analyze', type: 'analysis', description: 'Analyze file', status: 'completed' }
            ],
            findings: [],
            proposed_changes: []
          }
        ...
        file_glob_pattern: '**/*.al',
        phases: [{ id: 'analysis', name: 'Analysis', description: '', status: 'completed', mode: 'guided', required: true }],
        current_phase: 'analysis',
        current_file_index: 0,
        files_completed: 1,
        files_total: 1,
        ...
        file_inventory: [
          {
            ...
            checklist: [
              { id: 'analyze', type: 'analysis', description: 'Analyze file', status: 'completed' },
              { id: 'topic-1', type: 'topic_application', description: 'Apply SIFT patterns', status: 'pending', topic_id: 'dean-debug/sift-patterns' }
            ],
            findings: [],
            proposed_changes: []
          }
  ...
  describe('Checklist Expansion', () => {
    it('should expand checklist with new topic items', async () => {
        ...
        file_inventory: [
          {
            ...
            checklist: [
              { id: 'analyze', type: 'analysis', description: 'Analyze file', status: 'completed' },
              { id: 'validate', type: 'validation', description: 'Complete review', status: 'pending' }
            ],
            findings: [],
            proposed_changes: []
          }
      ...
      manager.expandChecklist(session, '/test/file.al', [
        { topic_id: 'dean-debug/sift-patterns', relevance_score: 0.9, description: 'SIFT optimization' },
        { topic_id: 'dean-debug/setloadfields', relevance_score: 0.8, description: 'SetLoadFields usage' }
      ]);
        ...
        file_inventory: [
          {
            ...
            status: 'in_progress',
            checklist: [
              { id: 'analyze', type: 'analysis', description: 'Analyze file', status: 'in_progress' },
              { id: 'validate', type: 'validation', description: 'Complete review', status: 'pending' }
            ],
            findings: [],
            proposed_changes: []
  ...
  describe('Report Generation', () => {
    it('should generate markdown report', async () => {
      const session: WorkflowSession = {
        ...
        files_total: 1,
        findings: [
          { file: '/test/file.al', line: 10, severity: 'warning', category: 'performance', description: 'Test warning' },
          { file: '/test/file.al', line: 20, severity: 'error', category: 'bug', description: 'Test error' }
        ],
        proposed_changes: [],
        options: {}
      };
  ...
  describe('Performance', () => {
    it('should handle large file inventories', async () => {
      const files = Array.from({ length: 100 }, (_, i) => ({
        ...
        checklist: [
          { id: `analyze-${i}`, type: 'analysis' as const, description: 'Analyze', status: 'pending' as const }
        ],
        findings: [],
        proposed_changes: []
      }));
...
describe('Custom Workflow Type Registration', () => {
  ...
  it('should register a custom workflow type', () => {
    const customWorkflow: WorkflowDefinition = {
      ...
      file_patterns: ['**/*.al'],
      phases: [
        { id: 'scan', name: 'Scan', description: 'Scan files', required: true, mode: 'autonomous' }
      ],
      per_file_checklist: [
        { id: 'check', type: 'analysis', description: 'Check file', required: true }
      ],
      topic_discovery: {
        enabled: false,
