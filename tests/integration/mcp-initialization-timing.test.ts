...
describe('MCP Server Initialization Performance (Issue #31)', () => {
  ...
  describe('Handshake timing', () => {
    it('should complete MCP handshake in under 5 seconds', async () => {
    ...
    it('should complete MCP handshake before heavy initialization', async () => {
      const result = await startServerAndMeasure();
    ...
    it('should not timeout during initialization (Windsurf issue)', async () => {
      const result = await startServerAndMeasure();
  ...
  describe('Startup sequence', () => {
    it('should log transport connection before service initialization', async () => {
      return new Promise((resolve) => {
        const proc = spawn('node', [serverPath], {
          stdio: ['pipe', 'pipe', 'pipe'],
          cwd: projectRoot
        ...
        setTimeout(() => {
          if (!events.includes('startup_complete')) {
            proc.kill('SIGKILL');
            expect.fail(`Timeout. Events: ${events.join(' -> ')}. Last output: ${stderrOutput.slice(-300)}`);
          }
        }, timeout);
      });
    }, timeout);
