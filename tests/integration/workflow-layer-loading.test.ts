...
describe('Workflow Loading from Layers (Issue #29)', () => {
  ...
  describe('Project layer workflow loading', () => {
    it('should load workflows from workflows/ directory', async () => {
      const { ProjectKnowledgeLayer } = await import('../../src/layers/project-layer');
...
`;
      ...
      await writeFile(join(workflowsPath, 'custom-review.yaml'), sampleWorkflow, 'utf-8');
...
`;
      ...
      await writeFile(join(methodologiesPath, 'legacy-workflow.yaml'), legacyWorkflow, 'utf-8');
...
`;
      ...
      await writeFile(join(workflowsPath, 'invalid.yaml'), invalidWorkflow, 'utf-8');
...
`;
      ...
      await writeFile(join(workflowsPath, 'malformed.yaml'), malformedYaml, 'utf-8');
  ...
  describe('Use case: Guided feature development workflow', () => {
    it('should support 7-phase workflow with hard stops', async () => {
      ...
      // Verify hard stops
      const hardStopPhases = guidedWorkflow.phases.filter((p: any) => p.hard_stop);
      expect(hardStopPhases.length).toBeGreaterThan(0);
      
      // Verify specialist handoffs
      const phasesWithHandoffs = guidedWorkflow.phases.filter((p: any) => p.handoff_to);
