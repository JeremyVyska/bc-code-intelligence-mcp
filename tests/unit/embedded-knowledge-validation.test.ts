...
describe("Embedded Knowledge Validation", () => {
  describe("Directory Structure", () => {
    ...
    it("should have required subdirectories", () => {
      ...
      const hasWorkflowsDir =
        existsSync(workflowsPath) || existsSync(methodologiesPath);
      expect(
        hasWorkflowsDir,
        `Missing directory: workflows/ or methodologies/`,
      ).toBe(true);
    });
  });
  ...
  describe("Critical Files", () => {
    it("should have workflow files", () => {
      ...
      expect(existsSync(workflowsDir), "Missing workflows/ directory").toBe(
        true,
      );
      ...
      expect(
        ...
        "Should have at least one workflow YAML file",
      ).toBeGreaterThan(0);
    });

    it("should have domain catalog", () => {
      const catalogPath = join(
        embeddedKnowledgePath,
  ...
  describe("Build Safety", () => {
    it("should prevent building with missing embedded knowledge", async () => {
      ...
      const validationScript = join(
        projectRoot,
        "scripts/validate-embedded-knowledge.ts",
      );
