...
export class EmbeddedKnowledgeLayer extends BaseKnowledgeLayer {
  ...
  override readonly supported_content_types: LayerContentType[] = [
    "topics",
    "specialists",
    "workflows",
  ];
  ...
  async initialize(): Promise<LayerLoadResult> {
    ...
    try {
      console.error(
        `Initializing ${this.name} layer from ${this.embeddedPath}...`,
      );
...
The embedded-knowledge directory should contain BC expertise (domains/ or topics/, specialists/, workflows/) required for the MCP server to function.
      ...
      const hasWorkflows =
        existsSync(join(this.embeddedPath, "workflows")) ||
...
Expected structure: (domains/ or topics/), specialists/, workflows/ directories with BC expertise content.
      ...
      const [topicsLoaded, specialistsLoaded, indexesLoaded] =
        await Promise.all([
          this.loadTopics(),
  ...
  protected async loadTopics(): Promise<number> {
    ...
    const basePath = useDomainsPath
      ? domainsPath
      : useTopicsPath
        ? topicsPath
        : domainsPath;
    const dirType = useDomainsPath
      ? "domains"
      : useTopicsPath
        ? "topics"
        : "domains (not found)";
    ...
    console.error(
      `   Directory exists: ${existsSync(basePath)} (using ${dirType})`,
    );
    ...
    try {
      ...
      if (topicFiles.length === 0) {
        console.error(
          `⚠️  Warning: No topic files found. Check if embedded-knowledge is properly populated.`,
        );
      }
      ...
      console.error(
        `❌ Fatal error loading topics from ${domainsPath}:`,
        error,
      );
      console.error(
  ...
  protected async loadIndexes(): Promise<number> {
    ...
    try {
      ...
      console.error(
        `Failed to load indexes from ${indexesPath}:`,
        error instanceof Error ? error.message : String(error),
      );
      return 0;
    }
    ...
    try {
      ...
      for (const tagFile of tagFiles) {
        ...
        try {
          ...
          console.error(
            `Failed to load tag index ${tagFile}:`,
            error instanceof Error ? error.message : String(error),
          );
        }
      }
  ...
  private async loadTopicRelationships(indexesPath: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Failed to load topic relationships:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  }
  ...
  private async loadBCVersionMatrix(indexesPath: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Failed to load BC version matrix:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  }
  ...
  protected async loadSpecialists(): Promise<number> {
    ...
    try {
      ...
      if (!specialistsStats.isDirectory()) {
        console.error(
          `⚠️ Specialists path is not a directory: ${specialistsPath}`,
        );
        return 0;
      }
    } catch (error) {
    ...
    console.error(
      `Found ${specialistFiles.length} specialist files in ${specialistsPath}`,
    );
    ...
    for (const filePath of specialistFiles) {
      try {
        ...
        console.error(
          `Failed to load specialist ${filePath}:`,
          error instanceof Error ? error.message : String(error),
        );
      }
    }
    ...
    try {
      ...
      const specialist: SpecialistDefinition = {
        ...
        persona: {
          ...
          communication_style:
            frontmatterData.persona?.communication_style || "",
          greeting:
        ...
        collaboration: {
          natural_handoffs:
            frontmatterData.collaboration?.natural_handoffs || [],
          team_consultations:
  ...
  protected async loadWorkflows(): Promise<number> {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `⚠️  Using deprecated 'methodologies/' directory. Please rename to 'workflows/'`,
        );
      } catch {
    ...
    console.error(
      `📋 Found ${workflowFiles.length} workflow files in ${activePath}`,
    );
    ...
    for (const filePath of workflowFiles) {
      try {
        ...
        console.error(
          `Failed to load workflow ${filePath}:`,
          error instanceof Error ? error.message : String(error),
        );
      }
    }
   ...
   */
  protected override searchSpecialists(
    query: string,
    limit?: number,
   ...
   */
  override hasContent<T extends LayerContentType>(
    type: T,
    id: string,
  ): boolean {
    switch (type) {
